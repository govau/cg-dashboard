...
// If one exists, do our best to make sure it's valid
func GetValidToken(req *http.Request, rw http.ResponseWriter, settings *Settings) *oauth2.Token {
	...
	// Did it change?
	if rv.AccessToken != token.AccessToken {
		// Hacky trick - we keep the *old* refresh token, as it may be an opaque short one,
		// which we need to keep small, else we won't fit this in a cookie.
		// We can't use opaque for the access token though, as Cloud Controller doesn't
		// support it.
		rv.RefreshToken = token.RefreshToken
