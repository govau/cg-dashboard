...
import ServiceInstanceTableRow from "./service_instance_table_row";
  ...
  get documentation() {
    return (
      <PanelDocumentation description>
        <p>
          To create service instances for this space, use this org’s marketplace
          (below or on the command line).
          {config.docs.managed_services && (
            <span>
              (<a href={config.docs.managed_services}>
                Learn about using service instances and marketplaces
              </a>.)
            </span>
          )}
        </p>
      ...
      content = (
        <div>
          ...
          <table>
            ...
            <tbody>{this.serviceInstances}</tbody>
          </table>
        </div>
      );
    }
